...
CREATE OR REPLACE TRIGGER VALID_INCOME
    BEFORE INSERT OF INCOME ON MOVIEINDEX.MOVIE
...
CREATE OR REPLACE TRIGGER VALID_COST
    BEFORE INSERT OF COST ON MOVIEINDEX.MOVIE
    REFERENCING NEW AS NEW_MOVIE
    FOR EACH ROW MODE DB2SQL
...
CREATE OR REPLACE TRIGGER VALID_DURATION
    BEFORE INSERT OF DURATION ON MOVIEINDEX.MOVIE
    REFERENCING NEW AS NEW_MOVIE
    FOR EACH ROW MODE DB2SQL
    WHEN (NEW_MOVIE.DURATION <= 0)
    BEGIN ATOMIC
